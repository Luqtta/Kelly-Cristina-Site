document.addEventListener('DOMContentLoaded', () => {
  ...
  function checkCenter() {
    ...
    if (window.innerWidth < 576 || window.innerWidth < 768) {
      ...
      cards.forEach((card) => {
        ...
        const tolerance = window.innerWidth < 576 ? 200 : 250;
        const verticalTolerance = window.innerWidth < 576 ? 100 : 150;

        if (
          isVisible &&
          (distanceToCenter < tolerance ||
            (rect.top < centerY + verticalTolerance && rect.bottom > centerY - verticalTolerance))
        ) {
  ...
  function renderReviews(reviews) {
    ...
    if (!carousel) {
      console.warn('Elemento com id "carousel" não encontrado.');
      return;
    }
